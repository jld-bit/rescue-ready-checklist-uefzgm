...
export default function HurricaneScreen() {
  ...
  const loadItems = async () => {
    try {
      console.log('Loading hurricane checklist items...');
  ...
  useEffect(() => {
    console.log('Hurricane screen: Setting up checklist event listener');
    const unsubscribe = checklistEvents.subscribe(CATEGORY, () => {
      console.log('Hurricane screen: Received reset event, reloading...');
      setTimeout(() => {
        loadItems();
      }, 100);
    });
    ...
    return () => {
      console.log('Hurricane screen: Cleaning up event listener');
      unsubscribe();
    };
  }, []);
  ...
  const saveItems = async (updatedItems: ChecklistItemType[]) => {
    try {
      ...
      console.log('Saved hurricane checklist items');
    } catch (error) {
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerContent}>
          ...
          <View style={styles.headerTextContainer}>
            <Text style={styles.headerTitle}>Hurricane</Text>
