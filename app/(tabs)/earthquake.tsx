...
export default function EarthquakeScreen() {
  ...
  useEffect(() => {
    console.log('Earthquake screen: Setting up checklist event listener');
    const unsubscribe = checklistEvents.subscribe(() => {
      console.log('Earthquake screen: Received reset event, reloading...');
      loadItems();
    });

    return () => {
      console.log('Earthquake screen: Cleaning up event listener');
      unsubscribe();
    };
  }, [loadItems]);
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              const resetItems = getDefaultItems('earthquake');
